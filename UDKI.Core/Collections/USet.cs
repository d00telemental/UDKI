...
public class USet<T, TKey, TKeyFuncs> : IEnumerable<T> where TKeyFuncs : struct, IKeyFuncs<T, TKey>
{
    ...
    private readonly USparseArray<SetElement> Elements;
    ...
    private void ClearHash()
    {
        ...
        MemoryMarshal.AsBytes(Hash.AsSpan()).Fill(byte.MaxValue);
    }
...
public struct DefaultKeyFuncs<T> : IKeyFuncs<T, T>
{
    ...
    public static int GetKeyHash(T? key) => key?.GetHashCode() ?? 0;
