...
using ThrowHelper = UDKI.Core.Helpers.ThrowHelper;

namespace UDKI.Core.Collections;

public interface IUMapInternal
{
    public bool TryAdd(object? key, object? value);
}

//C# version of Unreal's TMap
public abstract class UMapBase<TKey, TValue, TKeyFuncs> : IUMapInternal, IDictionary<TKey?, TValue?> where TKeyFuncs : struct, IKeyFuncs<KeyValuePair<TKey?, TValue?>, TKey?>
{
    ...
    public ICollection<TValue?> Values
    {
        get
        {
            ...
            foreach ((_, TValue? value) in Pairs)
            {
                vals.Add(value);
            }
            return vals;
    ...
    public void Add(TKey? key, TValue? value)
    {
        ...
        {
            CommunityToolkit.Diagnostics.ThrowHelper.ThrowArgumentException(nameof(key), "An element with the same key already exists in this map.");
        }
    }
    ...
    public bool TryAdd(object? key, object? value)
    {
        if (key is TKey || !typeof(TKey).IsValueType && key is null)
        {
            if (value is TValue || !typeof(TValue).IsValueType && value is null)
            {
...
public struct MapKeyFuncs<TKey, TValue> : IKeyFuncs<KeyValuePair<TKey, TValue?>, TKey>
{
    ...
    public static int GetKeyHash(TKey? key) => key?.GetHashCode() ?? 0;
